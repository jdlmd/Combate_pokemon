...
class MovimientoEstado : public Movimientos , public Estados {
    public:
        MovimientoEstado();
        MovimientoEstado(std::string _name, std::string _type, uint _atckORsp, uint _precision, uint _potencia, uint _pp, std::string _estado, uint _turnos, uint _porcentaje); // Constructor
        ~MovimientoEstado();
    private:
